...
export default function AdminCoursesClient() {
  ...
  async function loadCourses() {
    ...
    setCourses(j.data || []);
    setCourseId((j.data?.[0]?.id) || "");
  }

  async function loadAll(opts?: { keepModuleSelection?: boolean }) {
    if (!courseId) return;
    const r = await fetch(`/api/admin/courses/detail?course_id=${courseId}`, { cache: "no-store" });
    const j = await r.json();
    if (!r.ok) throw new Error(j.error || "Erro");

    const nextModules: Module[] = j.modules || [];
    const nextLessons: Lesson[] = j.lessons || [];

    setModules(nextModules);
    setLessons(nextLessons);

    // Só força o moduleId se:
    // - não tem moduleId ainda, OU
    // - o moduleId atual não existe mais, OU
    // - keepModuleSelection não foi pedido (troca de curso)
    const currentStillExists = nextModules.some((m) => m.id === moduleId);
    const shouldForce =
      !opts?.keepModuleSelection || !moduleId || !currentStillExists;
  ...
  useEffect(() => {
    // troca de curso: não mantém módulo
    loadAll({ keepModuleSelection: false }).catch(() => {});
  ...
  async function addModule(setToast: any) {
    ...
    try {
      const r = await fetch("/api/admin/modules", {
        ...
        body: JSON.stringify({ course_id: courseId, title: mTitle, description: mDesc, sort_order: mOrder }),
      });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Erro");

      setToast({ id: crypto.randomUUID(), text: "Módulo criado!", kind: "ok" });

      setMTitle("");
      setMDesc("");
      setMOrder(0);

      // Recarrega e seleciona o módulo recém-criado (se API retornar ele)
      await loadAll({ keepModuleSelection: true });
      if (j.data?.id) setModuleId(j.data.id);
    } catch (e: any) {
  ...
  async function delModule(id: string, setToast: any) {
    if (!confirm("Excluir módulo? Isso remove aulas do módulo.")) return;
    setLoading(true);
    try {
      const r = await fetch(`/api/admin/modules?id=${id}`, { method: "DELETE" });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Erro");
      setToast({ id: crypto.randomUUID(), text: "Módulo excluído!", kind: "ok" });
      await loadAll({ keepModuleSelection: false });
    } catch (e: any) {
      setToast({ id: crypto.randomUUID(), text: e.message, kind: "err" });
    } finally {
      setLoading(false);
  ...
  async function addLesson(setToast: any) {
    ...
    try {
      const r = await fetch("/api/admin/lessons", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ module_id: moduleId, title: lTitle, description: lDesc, sort_order: lOrder, link: lLink }),
      });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Erro");

      setToast({ id: crypto.randomUUID(), text: "Aula criada!", kind: "ok" });
      setLTitle("");
      setLDesc("");
      setLOrder(0);
      setLLink("");

      // mantém módulo selecionado
      await loadAll({ keepModuleSelection: true });
    } catch (e: any) {
      setToast({ id: crypto.randomUUID(), text: e.message, kind: "err" });
    } finally {
  ...
  async function delLesson(id: string, setToast: any) {
    if (!confirm("Excluir aula?")) return;
    setLoading(true);
    try {
      const r = await fetch(`/api/admin/lessons?id=${id}`, { method: "DELETE" });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Erro");
      setToast({ id: crypto.randomUUID(), text: "Aula excluída!", kind: "ok" });
      await loadAll({ keepModuleSelection: true });
    } catch (e: any) {
      setToast({ id: crypto.randomUUID(), text: e.message, kind: "err" });
    } finally {
        ...
        <>
          <div className="card" style={{ padding: 16, marginTop: 16 }}>
            <div className="row">
              ...
              <span className="badge">
                Cursos: <b>{courses.length}</b>
              </span>
            </div>
            <div className="small" style={{ marginTop: 10 }}>
              Cursos são itens do tipo <b>cursos</b> (crie em Admin → Itens).
            </div>
          </div>
          ...
          <div className="grid" style={{ marginTop: 16 }}>
            <div className="card" style={{ padding: 16 }}>
              ...
              <div className="row" style={{ marginTop: 10 }}>
                <button className="pill pillPrimary" disabled={loading} onClick={() => addModule(setToast)}>
                  {loading ? "Salvando..." : "Adicionar módulo"}
                </button>
              </div>
            </div>
            ...
            <div className="card" style={{ padding: 16 }}>
              ...
              <div className="row" style={{ marginTop: 10 }}>
                <button className="pill pillPrimary" disabled={loading} onClick={() => addLesson(setToast)}>
          ...
          <div className="card" style={{ padding: 16, marginTop: 16 }}>
            <div className="small">
              Módulos ({modules.length}) e Aulas ({lessons.length})
            </div>
            <div className="hr" />
            ...
            {modules.map((m) => (
              <div key={m.id} style={{ marginTop: 12 }}>
                <div className="row" style={{ alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    <b>{m.title}</b>
                    {m.description ? <div className="small">{m.description}</div> : null}
                  </div>
                  <button className="miniBtn miniBtnDanger" onClick={() => delModule(m.id, setToast)}>
                    Excluir módulo
                  </button>
                </div>
                  ...
                  {(lessonsByModule.get(m.id) || []).map((l) => (
                    <div key={l.id} className="card" style={{ padding: 12, background: "rgba(255,255,255,.03)" }}>
                      <div className="row" style={{ alignItems: "center", justifyContent: "space-between" }}>
                        <div>
                          <b>{l.title}</b>
                          {l.description ? <div className="small">{l.description}</div> : null}
                          {l.link ? <div className="small">Link: {l.link}</div> : null}
                        </div>
                        <button className="miniBtn miniBtnDanger" onClick={() => delLesson(l.id, setToast)}>
                          Excluir
                        </button>
                      </div>
                    </div>
